import { A as normalizeCredits, B as installedAppVersion, C as defaultDesktopAuthPath, D as WorkBuddyUpstreamClient, E as workbuddyOwnAuthPath, F as PROBE_EFFORT_CANDIDATES, H as resolveAppVersion, I as probeModel, L as randomSentinel, M as prepareChatBody, N as prepareInternationalChatBody, O as classifyUpstreamError, P as regionOf, R as WORKBUDDY_APP_VERSION_FILENAME, S as defaultDesktopAuthCandidates, T as parseWorkBuddyAuth, U as validAppVersion, V as readBundleVersion, _ as FALLBACK_WORKBUDDY_MODELS, a as WORKBUDDY_HOST_HEARTBEAT_FILENAME, b as WORKBUDDY_AUTH_FILE_ENV, c as processStartTimeMs, d as writeHostHeartbeat, g as FALLBACK_WORKBUDDY_AI_MODELS, i as variantFor, j as parseModelCatalog, k as modelWithCurrentPromotion, l as readHostHeartbeat, n as CN_VARIANT, o as clearHostHeartbeat, p as WORKBUDDY_CONNECT_VERSION, r as WORKBUDDY_VARIANTS, s as isHeartbeatProcessAlive, t as AI_VARIANT, u as workbuddyHostHeartbeatPath, v as WorkBuddyCatalog, w as desktopAuthCandidatesFor, x as WorkBuddyCredentialStore, y as WORKBUDDY_AUTH_FILENAME, z as appUserAgent } from "./variants-B8NbZTvg.js";
...
//#endregion
//#region src/context-windows.ts
/** Normalize the upstream list without inferring alternatives from a ceiling. */
function declaredContextWindows(info) {
	const declared = Array.isArray(info.supportedContextWindows) ? info.supportedContextWindows.filter((value) => Number.isSafeInteger(value) && value > 0) : [];
	return [...new Set(declared)].sort((a, b) => a - b);
}
/** Use only declared windows; a catalog without alternatives keeps its default. */
function supportedContextWindows(info) {
...
function resolveContextWindow(info, selected) {
	return selected !== void 0 && supportedContextWindows(info).includes(selected) ? selected : info.contextWindow;
}
//#endregion
...
function toPiModel(info, baseUrl, observed, providerId = WORKBUDDY_PROVIDER, contextWindow) {
	return {
		id: info.id,
		name: info.name,
...
function createWorkBuddyAdapter(options) {
	...
	const refreshProfiles = (force = false) => {
		const models = buildModels();
		const signature = JSON.stringify(models);
		if (force || signature !== modelSignature) {
			profiles = /* @__PURE__ */ new Map([[providerId, {
				...profile,
				piProvider: buildProvider(models)
			}]]);
...
async function workBuddyWebStatus(deps) {
	...
	const modelsField = deps.models().map((model) => {
		...
		return {
			...
			...Number.isSafeInteger(model.contextWindow) && model.contextWindow > 0 ? { defaultContextWindow: model.contextWindow } : {},
			...supported.length > 0 ? { supportedContextWindows: supported } : {},
			...maxContextWindow === void 0 || maxContextWindow === contextWindow ? {} : { maxContextWindow },
...
function parseAction(text) {
	...
	if (action === "context-window") {
		...
		if (typeof model !== "string" || model.trim() === "") return void 0;
		if (typeof contextWindow !== "number" || !Number.isSafeInteger(contextWindow) || contextWindow <= 0) return void 0;
		return {
	...
	return async (req, res) => {
		...
		try {
			...
			if (action.action === "context-window") {
				if (deps.setContextWindow === void 0) {
				...
				const contextWindow = action.contextWindow;
...
const MODEL_CONTEXT_WINDOWS_FIELD = z.dict(z.number().min(1).max(Number.MAX_SAFE_INTEGER).step(1)).description("Selected context capacity per model, in tokens");
...
function contextWindowsFieldFor(variant) {
	...
	*/
		...
		for (const runtime of runtimes) {
			...
			registerWorkBuddyProbeRoute(webCtx, {
				...
				setContextWindow: async (modelId, contextWindow) => {
					...
					if (stopped || settings === void 0 || !settings.writable) return false;
					await settings.update(settingsNamespaceFor(runtime.variant), { [contextWindowsFieldFor(runtime.variant)]: { [modelId]: contextWindow } });
					runtime.invalidate();
					return true;
				},
				probe: async (modelId) => {
	...
	ctx.inject(["settings"], (settingsCtx) => {
		...
		const merged = () => ({
			...
			...sources.cn().modelContextWindows === void 0 ? {} : { modelContextWindows: sources.cn().modelContextWindows },
			...sources.ai().authFileAI === void 0 ? {} : { authFileAI: sources.ai().authFileAI },
			...sources.ai().modelContextWindowsAI === void 0 ? {} : { modelContextWindowsAI: sources.ai().modelContextWindowsAI }
		});
		const repointStores = () => {
			const next = merged();
			for (const runtime of runtimes) {
